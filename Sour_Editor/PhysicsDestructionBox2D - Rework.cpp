...
static const int MAXSIZE = 10000; //400
...
public:
	...
	{
		auto pixelsToMeter = [&](b2Vec2& vertex) {return b2Vec2((vertex.x /= 10000), (vertex.y /= 10000)); };
		std::transform(vertices.begin(), vertices.end(), vertices.begin(), pixelsToMeter);
...
{
	static const int SIZEMAX = 10000;
	static const int SIZEMIN = 10000;
	static const float CENTERPOS = SIZEMAX - SIZEMIN;
	for (size_t i = 0; i < SEEDS; i++)
	{
		float x = float(rand() % SIZEMAX + SIZEMIN);
		float y = float(rand() % SIZEMAX + SIZEMIN);
		BWpoints.push_back(DBG_NEW HALF_EDGE::HE_Vertex(x, y, i));
	}

	//BWpoints.push_back(DBG_NEW HALF_EDGE::HE_Vertex(CENTER, CENTER, 0));
	{
		float minMax = SIZEMAX * 2;
		sf::Vector2f starTop = { CENTERPOS, CENTERPOS - minMax };
		sf::Vector2f starLeft = { CENTERPOS - minMax, CENTERPOS };
		sf::Vector2f starRight = { CENTERPOS + minMax, CENTERPOS };
		sf::Vector2f starBottom = { CENTERPOS, CENTERPOS + minMax };

		BWpoints.push_back(DBG_NEW HALF_EDGE::HE_Vertex(SIZEMIN, SIZEMIN, BWpoints.size()));
		BWpoints.push_back(DBG_NEW HALF_EDGE::HE_Vertex(SIZEMAX + SIZEMIN, SIZEMIN, BWpoints.size()));
		BWpoints.push_back(DBG_NEW HALF_EDGE::HE_Vertex(SIZEMAX + SIZEMIN, SIZEMAX + SIZEMIN, BWpoints.size()));
		BWpoints.push_back(DBG_NEW HALF_EDGE::HE_Vertex(SIZEMIN, SIZEMAX + SIZEMIN, BWpoints.size()));
...
int main()
{
	...
	Delunay::Triangulate(BWpoints, triangles);

	for (auto& vertex : BWpoints)
	{
		vertex->point->x /= 10000.0f;
		vertex->point->y /= 10000.0f;
	}
...
std::pair<sf::Vector2f, float> normalize(sf::Vector2f vector)
{
	float mx = vector.x;
	float my = vector.y;
	...
	return std::make_pair(sf::Vector2f(mx, my), multiplier);
}
