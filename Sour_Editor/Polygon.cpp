...
void Polygon::createPhysicsBody(std::vector<b2Vec2>& vertices)
{
	...
	{
		vertCount += test[i];
		std::vector<b2Vec2>::const_iterator first = vertices.begin() + std::fmin(vertCount - test[i], test.size()-8);
...
void Polygon::update()
{
	...
	auto newPos = sf::Vector2f(this->m_body->GetPosition().x * 10000, this->m_body->GetPosition().y * 10000);
