	...
	while(timer < 3)
	{
		...
		if(!pause)
		{
			for(auto& face : faces)
			{
				...
				point.setPosition(sf::Vector2f(face->circumCenter.x, face->circumCenter.y) -
								  sf::Vector2f(face->radius, face->radius));
...
void Delunay::Triangulate(std::vector<HALF_EDGE::HE_Vertex*>& points,
						  std::vector<HALF_EDGE::HE_Face*>& faces)
{
		...
		/* Update triangulation. */
		{
			...
			for(auto edge : edges)
			{
				...
				std::pair<int, int> edge2 =
					std::make_pair(pt->arrayIndex, edge->twin->vert->arrayIndex);
				...
				std::pair<unsigned int, unsigned int> otherPair =
					std::make_pair(pt->arrayIndex, edge->vert->arrayIndex);
	...
	auto is_part_STriangle = [&](HALF_EDGE::HE_Face* const& face) {
		return (face->edge->vert->arrayIndex < 0 || face->edge->next->vert->arrayIndex < 0 ||
				face->edge->prev->vert->arrayIndex < 0);
	};
...
bool Delunay::findIntersections(HALF_EDGE::HE_Vertex* pt,
								std::vector<HALF_EDGE::HE_Face*> faces,
								std::map<HALF_EDGE::HE_Face*, bool>& intersections,
								std::vector<HALF_EDGE::HE_Edge*>& edges)
{
...
void Delunay::neighborIntersections(HALF_EDGE::HE_Vertex* pt,
