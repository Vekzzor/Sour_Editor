...
struct VoronoiCell
{
	...
	VoronoiCell(sf::Vector2f* sitePos)
		: site{sitePos} {};
};

void addVoronoiEdge(HALF_EDGE::HE_Edge* e,
					sf::Vector2f center,
					std::vector<VoronoiEdge*>& voronoiEdges)
{
	...
	voronoiEdges.push_back(
		DBG_NEW VoronoiEdge(center, voronoiVertexNeighbor, e->prev->vert->point));
}
...
void CreateSeedPoints(std::vector<HALF_EDGE::HE_Vertex*>& BWpoints, float _MaxSize, float _MinSize)
{
	...
	{
		BWpoints.push_back(
			DBG_NEW HALF_EDGE::HE_Vertex(_MinSize, _MinSize, BWpoints.size())); //TopLeft
		BWpoints.push_back(DBG_NEW HALF_EDGE::HE_Vertex(
			_MaxSize + _MinSize, _MinSize, BWpoints.size())); //TopRight
		BWpoints.push_back(DBG_NEW HALF_EDGE::HE_Vertex(
			_MaxSize + _MinSize, _MaxSize + _MinSize, BWpoints.size())); //BottomRight
		BWpoints.push_back(DBG_NEW HALF_EDGE::HE_Vertex(
			_MinSize, _MaxSize + _MinSize, BWpoints.size())); //BottomLeft
	}
...
void CreateSeedPoints(std::vector<HALF_EDGE::HE_Vertex*>& BWpoints,
					  sf::Vector2f CornerMax,
					  sf::Vector2f CornerMin)
{
	...
	if(BWpoints.empty())
	{
		...
		BWpoints.push_back(
			DBG_NEW HALF_EDGE::HE_Vertex(CornerMax.x, CornerMin.y, BWpoints.size())); //TopRight
		BWpoints.push_back(DBG_NEW HALF_EDGE::HE_Vertex(CornerMax, BWpoints.size())); //BottomRight
		BWpoints.push_back(
			DBG_NEW HALF_EDGE::HE_Vertex(CornerMin.x, CornerMax.y, BWpoints.size())); //BottomLeft
	}
...
bool LiangBarsky(double edgeLeft,
				 ...
				 sf::Vector2f src0,
				 sf::Vector2f src1, // Define the start and end points of the line.
				 sf::Vector2f& clip0,
				 sf::Vector2f& clip1) // The output values, so declare these outside.
{
...
void dac_bp_Test(const std::vector<Polygon>& bounds,
...
void CreateVoronoiPolyBodies(std::vector<HALF_EDGE::HE_Vertex*>& _BWpoints,
							 std::vector<HALF_EDGE::HE_Face*>& _faceList,
...
{
	coordinator.Init();
	coordinator.RegisterComponent<Polygon>();
	...
	Signature signature;
	signature.set(coordinator.GetComponentType<Polygon>());
	...
	//coordinator.RegisterComponent<sf::Drawable>();
	...
	for(int i = 0; i < plane.getPointCount(); i++)
		plane.setPoint(i,
					   sf::Vector2f(boxShape.m_vertices[i].x * pixelToMeterRatio,
									boxShape.m_vertices[i].y * pixelToMeterRatio));
	plane.setOrigin(0, 0);
	plane.setPosition(staticBody->GetPosition().x * pixelToMeterRatio,
					  staticBody->GetPosition().y * pixelToMeterRatio);
	...
	std::deque<std::pair<Polygon, Selectable>> trashedBoxes;
	...
	for(auto& polygon : polygons)
	{
		...
		coordinator.AddComponent(entities.back(), Selectable{});
	}
	...
	IM_Instance->addFunctionToKey(std::bind(&funcPointTest),
								  sf::Keyboard::Q,
								  InputMapping::KeyState::Pressed,
								  InputMapping::Shift | InputMapping::Alt);
	IM_Instance->addFunctionToKey([&]() { window.close(); }, sf::Keyboard::Escape);
	IM_Instance->addFunctionToKey(
		[&]() {
			if(tools.getSelectedObject() != -1)
				coordinator.GetComponent<Polygon>(tools.getSelectedObject())
					.m_body->SetLinearVelocity({10, 0});
		},
		sf::Keyboard::D);
	IM_Instance->addFunctionToKey(
		[&]() {
	...
	IM_Instance->addFunctionToKey(
		[&]() {
			if(!entities.empty())
			{
				auto& selectable = coordinator.GetComponent<Selectable>(entities.back());
				auto& polygon	= coordinator.GetComponent<Polygon>(entities.back());
				polygon.m_body->SetEnabled(false);
				trashedBoxes.push_back(std::make_pair(polygon, selectable));
				coordinator.DestroyEntity(entities.back());
				entities.pop_back();
			}
		},
		sf::Keyboard::Z,
	...
	IM_Instance->addFunctionToKey(
		[&]() {
			...
			{
				trashedBoxes.back().first.m_body->SetEnabled(true);

				entities.push_back(coordinator.CreateEntity());
				coordinator.AddComponent(entities.back(), trashedBoxes.back().first);
				coordinator.AddComponent(entities.back(), trashedBoxes.back().second);
	...
	IM_Instance->addFunctionToMouseKey(
		[&]() { moving = false; }, sf::Mouse::Button::Middle, InputMapping::KeyState::Released);
	...
	IM_Instance->addFunctionToMouseKey(
		[&]() {
			...
			dac_bp_Test(boxes, indices.begin(), indices.end(), intersections, { pos.x,pos.y });*/
				...
				if(!entities2.empty())
				{
					tools.setSelectedObject(entities2.back());
		...
		sf::Mouse::Button::Left,
		InputMapping::KeyState::Released);
	...
	while(window.isOpen())
	{
		...
		if(tools.getSelectedObject() != -1)
		{
			Entity entity = tools.getSelectedObject();
			auto& polygon = coordinator.GetComponent<Polygon>(entity);
			b2Vec2 pos	= polygon.m_body->GetPosition();

			ImGui::Begin("SelectedObject");
			ImGui::SliderFloat("X", &pos.x, -10, 10.0f);
			ImGui::SliderFloat("Y", &pos.y, -10, 10.0f);
			if(pos != polygon.m_body->GetPosition())
			{
				polygon.m_body->SetTransform(pos, polygon.m_body->GetAngle());
				polygon.m_body->SetAwake(true);
			}
			ImGui::End();
		}
							 ...
							 std::vector<Polygon>& _polygons)
{
	...
	for(auto& cell : voronoiCells)
	{
		...
		for(int i = 0; i < cell.edges.size(); i++)
		{
			...
			if(LiangBarsky(midx - halfwidthX,
						   midx + halfwidthX,
						   midy + halfwidthY,
						   midy - halfwidthY,
						   test[1].position,
						   ...
						   v2Result))
			{
				...
				if(v1Result != test[1].position)
				{
								   ...
								   v2ResultN))
					{
						...
						physicsVertices.push_back(b2Vec2((v1Result.x / pixelToMeterRatio),
														 (v1Result.y / pixelToMeterRatio)));
						physicsVertices.push_back(b2Vec2((v2ResultN.x / pixelToMeterRatio),
														 (v2ResultN.y / pixelToMeterRatio)));
					}
				}
				if(v2Result != test[2].position)
				{
								   ...
								   v2ResultN))
					{
						...
						physicsVertices.push_back(b2Vec2((v2Result.x / pixelToMeterRatio),
														 (v2Result.y / pixelToMeterRatio)));
						physicsVertices.push_back(b2Vec2((v2ResultN.x / pixelToMeterRatio),
														 (v2ResultN.y / pixelToMeterRatio)));
					}
				}
			...
			if(draw)
			{
				...
				physicsVertices.push_back(b2Vec2((test[1].position.x / pixelToMeterRatio),
												 (test[1].position.y / pixelToMeterRatio)));
				physicsVertices.push_back(b2Vec2((test[2].position.x / pixelToMeterRatio),
												 (test[2].position.y / pixelToMeterRatio)));
			}
		}
		...
		if(polygon.m_vertices.size() != 0)
		{
			...
			polygon.m_center.setPosition(
				sf::Vector2f(cell.site->x, cell.site->y) -
				sf::Vector2f(polygon.m_center.getRadius(), polygon.m_center.getRadius()));
